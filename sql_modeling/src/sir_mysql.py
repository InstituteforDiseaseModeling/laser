...
conn = None # mysql.connector.connect(**config)
...
def initialize_database(): # conn=None ):
    ...
    cursor.executemany('INSERT INTO agents VALUES (%s, %s, %s, %s, %s, %s, %s, %s)', agents_data)
    ...
    query = f'''
        UPDATE agents 
        SET infected = 1,
            infection_timer=FLOOR( 4+10*RAND() ),
            incubation_timer=3
        WHERE id IN (
            SELECT id
            FROM ( SELECT id FROM agents WHERE node={num_nodes-1} ORDER BY RAND() LIMIT 100)
            AS subquery
        )
    '''
...
def progress_infections( cursor ):
    ...
    cursor.execute( "UPDATE agents SET infected=0, immunity=1, immunity_timer=10+FLOOR( 30*RAND() ) WHERE infected=1 AND infection_timer=0" )
    return cursor # for pattern
...
def calculate_new_infections( cursor, inf, sus ):
    ...
    results = cursor.execute('SELECT node, COUNT(*) FROM agents WHERE incubation_timer >= 1 GROUP BY node')
...
def add_new_infections( cursor ):
    cursor.execute( "UPDATE agents SET infection_timer=4+FLOOR( 10*RAND() ) WHERE infected=1 AND infection_timer=0" )
    return cursor # for pattern
...
def migrate( cursor, timestep, **kwargs ): # ignore kwargs
    ...
    if timestep % 7 == 0: # every week (or day, depending on what I've set it to)
        print( "Something about the migration thing is really slow in mysql but it took way too long to just make this work..." )

        #cursor.execute("SELECT MAX(node) FROM agents WHERE infected = 1")
        #max_node = cursor.fetchone()[0]

        # Get the count of rows where infected is equal to 1
        cursor.execute("SELECT COUNT(*) FROM agents WHERE infected = 1") # we should have this from total infection count
