...
update_ages_lib = ctypes.CDLL(Path(__file__).parent / "update_ages.so")
...
def load(pop_file):
    ...
    columns["id"] = columns["id"].astype(np.uint32)
...
def progress_infections(data):
    ...
    def progress_infections_c(data):
        update_ages_lib.progress_infections(
            len(data["age"]), data["infection_timer"], data["incubation_timer"], data["infected"], data["immunity_timer"], data["immunity"]
        )
        return
...
def calculate_new_infections(data, inf, sus):
    def calculate_new_infections_c(data, inf, sus):
        ...
        update_ages_lib.calculate_new_infections(
            len(data["age"]), len(inf), data["incubation_timer"], data["node"], inf_np, sus_np, new_infections
        )
...
def migrate(data, timestep, num_infected=None):
    ...
    if timestep % 7 == 0:  # every week
        ...
        def migrate_c():
            update_ages_lib.migrate(len(data["age"]), data["infected"], data["node"])
...
if __name__ == "__main__":
    ...
    print(f"Initialization time: {tstart - tinit}")
    print(f"Simulation time: {tfinish - tstart}")
    print(f"Total time: {tfinish - tinit}")
