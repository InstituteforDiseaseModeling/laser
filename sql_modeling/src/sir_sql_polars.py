...
def calculate_new_infections( df, inf, sus ):
    ...
    sus_np = [value for key, value in sorted(sus.items())]
...
def _handle_transmission_inner( df, new_infections, node=0 ):
    ...
    if new_infections > len( df.filter(condition) ):
        ...
        pdb.set_trace()
    sampled_df = df.filter(condition).sample( new_infections )
    # Should we add a newly_infected=True col/flag for these guys so make them easy to catch?
    sampled_df = sampled_df.with_columns( infected=1, incubation_timer=2 )
...
def add_new_infections( df ):
    # Part of API but this is already done as part of handle_tx in this implementation
    return df

def migrate( df, timestep, **kwargs ):
    ...
    if timestep % 7 == 0: # every week (or day, depending on what I've set it to)
        ...
        sampled_df = df.filter(condition).sample( int( kwargs["num_infected"]*0.01 ) )
