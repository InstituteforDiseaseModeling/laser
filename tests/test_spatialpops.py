...
def test_distribute_population_skewed_alternative():
    np.random.seed(42)  # Set seed for reproducibility
    result = distribute_population_skewed(tot_pop=500, num_nodes=3, frac_rural=0.4)
...
def test_distribute_population_skewed_invalid_fraction():
    with pytest.raises(ValueError, match="Fraction of rural population must be between 0 and 1."):
        distribute_population_skewed(tot_pop=1000, num_nodes=5, frac_rural=-0.1)
    with pytest.raises(ValueError, match="Fraction of rural population must be between 0 and 1."):
        distribute_population_skewed(tot_pop=1000, num_nodes=5, frac_rural=1.5)
