...
def set_params():
    ...
    args.__setattr__("beta", np.float32(args.r_naught / args.mean_inf))
...
def test_sir(params):
    ...
    community = abc(params.pop_size,
                    ...
                    mean=params.mean_inf,                                              # add the mean property to the community
                    std=params.std_inf)                                                # add the std property to the community
    ...
    community.itimer[initial_infs] = prng.normal(params.mean_inf, params.std_inf, size=params.initial_inf).round().astype(ITIMER_TYPE_NP)
    ...
    @nb.njit((     SUSCEPTIBILITY_TYPE_NB[:], ITIMER_TYPE_NB[:], nb.uint32, nb.float32, nb.float32, nb.float32), parallel=True)
    ...
    @nb.njit((nb.uint32, SUSCEPTIBILITY_TYPE_NB[:], ITIMER_TYPE_NB[:], nb.uint32[:,:]), parallel=True)
    ...
    for timestep in tqdm(range(params.timesteps)):
        ...
        record(timestep+1, community.susceptibility, community.itimer, results) # record(timestep+1, community=community, results=results)
    ...
    print("Results written to 'sir.csv'.")
