...
class TestPredictedDateOfDeath(unittest.TestCase):
    def test_predicted_year_of_death_0_default(self):
        age = 0
        for _ in range(128):  # 100-ish trials
    ...
    def test_doy_distribution(self):
        for _ in range(128):
            ...
            if age_days % 365 == 364:
                assert dod % 365 == 0, f"Expected {dod} % 365 == 0"
            elif dod // 365 != age_days // 365:
                assert dod % 365 in range(365), f"Expected {dod} % 365 in range(0, 365)"
            else:
                assert dod % 365 in range(age_days % 365 + 1, 365), f"Expected {dod} % 365 in range({age_days % 365 + 1}, 365)"
