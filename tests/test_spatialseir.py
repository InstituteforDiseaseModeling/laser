...
def test_spatial_seir(params):
    ...
    # from dictionary with key and values ((population, year), (lat, lon), area) set total = sum(population)
    POP_INDEX = 0
    SIZE_INDEX = 0
    ...
    @nb.njit((SUSCEPTIBILITY_TYPE_NB[:], nb.uint16[:], nb.float32[:], ITIMER_TYPE_NB[:], nb.uint32, nb.float32, nb.float32), parallel=True)
    ...
    def transmission(community, timestep):
        ...
        transfer = (contagion * gravity).round().astype(np.uint32)          # contagion * gravity = transfer
        ...
        tx_inner(community.susceptibility, community.nodeid, forces, community.etimer, community.count, community.exp_mean, community.exp_std)
    ...
    # record initial state, state _after_ timestep i processing will be in index i
