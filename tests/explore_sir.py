...
"""Explore the final attack fraction of the SIR model."""
...
def batch(meta):
    ...
    if meta.plot:
        ...
        for filename in meta.resultsdir.glob("*.csv"):
            ...
            final = df.iloc[-1]
            fraction = final.recovered / (final.susceptible + final.infected + final.recovered)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resultsdir", type=Path, default=Path(__file__).parent / "results_sir")
    parser.add_argument("--min_r0", type=float, default=0.5)
    parser.add_argument("--max_r0", type=float, default=2.5)
    parser.add_argument("--steps", type=int, default=200)
    parser.add_argument("--replicates", type=int, default=5)
    parser.add_argument("--plotname", default="sir_scatter.png")
