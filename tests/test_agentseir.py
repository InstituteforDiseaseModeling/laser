...
def test_seir(params):
    ...
    @nb.njit((        ITIMER_TYPE_NB[:], ITIMER_TYPE_NB[:], nb.uint32, nb.float32, nb.float32), parallel=True)
    ...
    @nb.njit((  SUSCEPTIBILITY_TYPE_NB[:], ITIMER_TYPE_NB[:], ITIMER_TYPE_NB[:], nb.uint32, nb.float32, nb.float32, nb.float32), parallel=True)
    ...
    def transmission(community, _timestep):
        ...
        # community.susceptibility[infected] = 0
        # community.etimer[infected] = np.random.normal(MEAN_EXP, STD_EXP, size=infected.sum()).round().astype(ITIMER_TYPE_NP)

        transmission_inner(community.susceptibility, community.etimer, community.itimer, community.count, community.beta, community.exp_mean, community.exp_std)
    ...
    @nb.njit((nb.uint32, SUSCEPTIBILITY_TYPE_NB[:], ITIMER_TYPE_NB[:], ITIMER_TYPE_NB[:], nb.uint32[:,:]), parallel=True)
    def record(timestep,            susceptibility,            etimer,            itimer,        results):
        ...
        # results[timestep,4] = ((community.susceptibility == 0) & (community.etimer == 0) & (community.itimer == 0)).sum()
    ...
    for timestep in tqdm(range(params.timesteps)):
        ...
        record(timestep+1, community.susceptibility, community.etimer, community.itimer, results)   # record(timestep+1, community=community, results=results)
    ...
    print("Results written to 'seir.csv'.")
